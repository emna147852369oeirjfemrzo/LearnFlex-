   ...
   public function searchOrganismes(array $criteria = []): array
    {
        ...
        if (!empty($criteria['fraismin'])) {
            $qb->andWhere('o.fraismin = :taille')
               ->setParameter('fraismin', $criteria['fraismin']);
        }

        $direction = (isset($criteria['direction']) && strtoupper($criteria['direction']) === 'DESC') ? 'DESC' : 'ASC';

        if (!empty($criteria['sort']) && $criteria['sort'] === 'fraismin') {
            $qb->orderBy('o.fraismin', $direction);
        } else {
